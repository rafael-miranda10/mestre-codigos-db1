    ...
    public class Parametros : IParametros
    {
        ...
        public void ExibirExplicacao()
        {
            Console.WriteLine("As palavras reservadas ref e out são usadas para passar argumento/parâmetro para um método ou função. Indica que um argumento/parâmetro é passado por referência.\n" +
                "Simplificadamente um parâmetro normal tem um sentido, ele só vai, o valor do argumento só é enviado para o parâmetro. O out tambémsó tem um sentido, mas contrário, ele só passa o valor do parâmetro(o que está dentro do método) para o argumento.\n"+
                ...
                "Indica que o argumento a ser passado receberá um valor dentro do método. Ou seja, é uma referência também, mas não é passado nenhum valor para o parâmetro, é apenas umaforma de dar saída para um valor. Isto normalmente é necessário porque o return só pode ter um valor.\n"+
